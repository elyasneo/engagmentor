...
class Instagram:
    ...
    def progress(self,mCount, mTotal, mStatus=''):
        bar_len = 60
        filled_len = int(round(bar_len * mCount / float(mTotal)))

        percents = round(100.0 * mCount / float(mTotal), 1)
        bar = '=' * filled_len + '-' * (bar_len - filled_len)

        sys.stdout.write('   [%s] %s%s ...%s\r' % (bar, percents, '%', mStatus
))
        sys.stdout.flush()
    ...
    def get_medias_by_user_id(self, id, count=12, max_id=''):
        ...
        while index < count and is_more_available:
            ...
            if not Instagram.HTTP_OK == response.status_code:
                # raise InstagramException.default(response.text,
                #                                  response.status_code)
                print(f"\nSome Error Occurred!!\nWe got {len(medias)} posts")
                ans = input("Do you want to continue? (y/n): ")
                if ans=='y' or ans=='Y':
                    print("please enter another account to continue")
                    while True:
            ...
            self.progress(len(medias),count,"ALT3...")
